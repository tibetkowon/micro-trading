      ...
      {resultStocks.length > 0 ? (
        <div>
          <p className="text-xs text-th-on-subtle mb-1.5">
            {isOR ? 'OR 합집합' : 'AND 교집합'} — {types.join(separator)} — {resultStocks.length}종목
          </p>
          <div className="flex flex-wrap gap-1.5">
            {resultStocks.map((s) => (
              <span key={s.stock_code} className="inline-flex items-center gap-1 text-xs bg-th-surface-high border border-th-outline rounded-md px-2 py-0.5">
        ...
        <p className="text-xs text-th-loss">{log.error_message}</p>
      ) : (
    ...
    <div className="space-y-1.5">
      {filtered.map((f) => (
        <div key={f.stock_code} className="flex items-center gap-2 flex-wrap">
          ...
          <span className="badge bg-th-warn/10 text-th-warn border-th-warn/20 text-xs shrink-0">{f.filter_reason}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {hasSelected ? (
          <span className="badge bg-th-growth/10 text-th-growth border-th-growth/20">
            ✓ {selLog.selected_code} 선정
          </span>
        ) : hasFailed ? (
          <span className="badge bg-th-loss/10 text-th-loss border-th-loss/20">선정 실패</span>
        ) : (
          <span className="badge bg-th-warn/10 text-th-warn border-th-warn/20">적합 종목 없음</span>
        )}
      ...
      {hasFailed && (
        <p className="text-xs text-th-loss">{selLog.fail_reason}</p>
      )}
        ...
        <details>
          ...
          <div className="mt-2 overflow-x-auto">
            <table className="text-xs w-full border-collapse">
              <thead>
                <tr className="text-th-on-subtle border-b border-th-outline">
                ...
                {candidates.map((c) => {
                  ...
                  return (
                    <tr key={c.stock_code} className="text-th-on-muted">
                      ...
                      <td className={`py-1 pr-3 text-right font-data ${dm5 > 3 ? 'text-th-loss' : dm5 > 1.5 ? 'text-th-warn' : ''}`}>
                        {dm5 != null && dm5 !== 0 ? dm5.toFixed(1) + '%' : '-'}
                      </td>
                      <td className={`py-1 pr-3 text-right font-data ${hpd < -3 ? 'text-[#3B82F6]' : hpd > -0.5 ? 'text-th-loss' : ''}`}>
                        {hpd != null && hpd !== 0 ? hpd.toFixed(1) + '%' : '-'}
                      </td>
                      <td className={`py-1 pr-3 text-right font-data ${opd > 10 ? 'text-th-loss' : opd > 3 ? 'text-th-warn' : ''}`}>
...
function RankingCard({ log, selLog }) {
  ...
  return (
    <div className="bg-th-surface border border-th-outline rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          {log.market === 'US' ? (
            ...
            <span className="badge bg-th-surface-high text-th-on-muted border-th-outline shrink-0">국장</span>
          )}

          {hasError ? (
            <span className="badge bg-th-loss/10 text-th-loss border-th-loss/20 shrink-0">오류</span>
          ) : noMatch ? (
            <span className="badge bg-th-warn/10 text-th-warn border-th-warn/20 shrink-0">적합 없음</span>
          ) : (
            <span className="badge bg-th-growth/10 text-th-growth border-th-growth/20 font-data shrink-0">
              {isOR ? '합집합' : '교집합'} {log.intersection_count}종목
            </span>
          )}

          {filteredStocks.length > 0 && (
            <span className="badge bg-th-warn/10 text-th-warn border-th-warn/20 font-data shrink-0">
          ...
          {selLog && (
            selLog.selected_code ? (
              ...
              <span className="badge bg-th-loss/10 text-th-loss border-th-loss/20 shrink-0">선정 실패</span>
            ) : null
          )}
      ...
      {/* 상세 3단계 패널 */}
      {open && (
        <div className="px-4 pb-4 border-t border-th-outline space-y-5 pt-4">
          <StagePanel
            step={1}
            title="순위 조회"
            badge={
          ...
          <div className="border-t border-th-outline" />
          ...
          <StagePanel
            ...
            badge={
              filteredStocks.length > 0
                ? <span className="badge bg-th-warn/10 text-th-warn border-th-warn/20">{filteredStocks.length}종목 제거</span>
                : <span className="badge bg-th-growth/10 text-th-growth border-th-growth/20">전체 통과</span>
            }
          >
            ...
            badge={
              selLog
                ? selLog.selected_code
                  ...
                  : <span className="badge bg-th-loss/10 text-th-loss border-th-loss/20">미선정</span>
                : null
            }
          >
...
export default function StockLogs() {
  ...
  return (
    <div className="space-y-5">
      ...
      {rankingError && (
        <div className="bg-th-loss/10 border border-th-loss/20 text-th-loss rounded-xl p-4 text-sm">{rankingError}</div>
      )}

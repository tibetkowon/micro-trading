...
func GetLocalOrderHistory(ctx context.Context, db *database.DB, limit, offset int) ([]models.Order, error) {
	rows, err := db.QueryContext(ctx,
		`SELECT id, stock_code, stock_name, order_type, qty, price, filled_price, status, kis_order_id, source, sell_reason, COALESCE(market, 'KR'), created_at
	...
	for rows.Next() {
		var o models.Order
		if err := rows.Scan(&o.ID, &o.StockCode, &o.StockName, &o.OrderType, &o.Qty, &o.Price, &o.FilledPrice, &o.Status, &o.KISOrderID, &o.Source, &o.SellReason, &o.Market, &o.CreatedAt); err != nil {

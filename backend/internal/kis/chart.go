...
// GetDayMinuteChart fetches all 1-minute bars for the specified trading day in a single
// API call (up to 120 bars). Prefer this over GetMinuteChart for current-session indicator
// calculation — it reduces KIS API calls from 4+ pages to 1.
//
// date: YYYYMMDD (e.g. "20260508")
// inputHour: HHMMSS end time; pass "" to use current time.
// Bars are returned newest-first.
//
// NOTE: TR_ID FHKST03010230 — verify against KIS developer portal if this returns an error.
func (c *Client) GetDayMinuteChart(ctx context.Context, stockCode, date, inputHour string) ([]ChartBar, error) {
	if inputHour == "" {
		inputHour = time.Now().Format("150405")
	}
	if date == "" {
	...
	params := fmt.Sprintf(
		"?FID_COND_MRKT_DIV_CODE=J&FID_INPUT_ISCD=%s&FID_INPUT_DATE_1=%s&FID_INPUT_HOUR_1=%s&FID_PW_DATA_INCU_YN=Y&FID_FAKE_TICK_INCU_YN=N",
		stockCode, date, inputHour,
	)

	raw, err := c.get(ctx, endpoint, params, "FHKST03010230")
	if err != nil {
		return nil, err
	}
	...
	if result.MsgCode == "EGW00201" || (result.MsgCode != "" && result.MsgCode != "0" && len(result.Output2) == 0) {
		return nil, fmt.Errorf("KIS API error %s: %s", result.MsgCode, result.Msg)
	}
